...
- **`webcam_sop.py`에 연결 완료(2026-09-15)** — `--fcm-token`(또는 `DEVICE_TOKEN` 환경변수)을
  ...
  규칙별 등급(`FCM_LEVEL_BY_RULE`: 4규칙 전부 중대, 2026-09-22부로 helmet/glasses도 승격 —
  아래 "다음 작업" 해당 날짜 항목 참고)도 같이 보내 앱의 등급 배지에 반영됨.
  RepeatThrottle 게이트도 Web Push와 동일하게 적용(반복 위반 시 억제).
  **(2026-09-21 변경)** 이때는 등급과 무관하게 4규칙 전부 폰까지 발송했으나, 기획안 5.5절
  "관리자 푸시는 중대 편차에 한정한다"는 규정과 어긋남이 뒤늦게 발견돼 주의(헬멧·보안경)는
  폰 발송을 껐다 — 아래 "알림 설계" 섹션과 "다음 작업" 해당 날짜 항목 참고.
...
채널)는 이미 있다:
1. **등급별 차등 발송 — 재발송은 재시도 보강으로 축소, 주의는 폰 발송 자체를 차단
   (2026-09-21, 2차 수정)** — 기획안 그림 4 목업의 중대/일반/주의 3등급 처리를 다시 맞춤.
   - **중대(2인1조·안전구역)**: 기획안 5.5절 "확인할 때까지 30초 간격 재발송" 스펙은, 앱이
     이미 로컬에서 TTS·경보음을 반복 재생하고 있어 **서버가 같은 메시지를 다시 보내는 게
     실질적 이득이 거의 없다는 걸 확인**하고 방향을 바꿈. 서버 재발송이 의미 있는 유일한
     경우(최초 발송 자체가 폰에 안 닿음)에 대응해 `send_fcm()`에 실패 시 재시도만 추가
     (`_try_send_fcm_once`, 최대 3회). 원래 계획했던 백그라운드 스케줄러·ACK 엔드포인트·
     서비스워커 액션 버튼은 불필요해짐.
   - **주의(헬멧·보안경)**: 위 재시도 작업 직후 기획안 5.5절을 다시 읽다가 "오경보는 작업
     방해와 시스템 신뢰 저하를 유발하므로 **관리자 푸시는 중대 편차에 한정한다**"는 규정을
     재발견 — 그런데 실제 코드는 등급 무관하게 4규칙 전부 폰에 발송하고 있었다(`FCM_LEVEL_BY_RULE`은
     표시용일 뿐 발송 여부를 안 가르는 버그). `on_violation_confirmed`에서 `level == "중대"`일
     때만 `send_fcm`을 호출하도록 고쳐서 주의 등급은 이제 `events.jsonl`에는 남지만 폰까지는
     안 간다 — 기획안 등급표(안내음/이벤트 기록만, 푸시 미발송)와 일치.
   - **일반(표준시간 초과)**: 대상 규칙 자체가 아직 없음(SOP 필요, 해당 없음).
   - 상세는 아래 "다음 작업" 2026-09-21 항목.
...
- [x] **알림 등급별 차등 발송 재검토 → `send_fcm()` 재시도 보강으로 축소 (2026-09-21)** —
  "규모 큼"이라 미루던 항목을 다시 설계하다가, 기획안 5.5절의 "중대는 확인할 때까지 30초
  간격 재발송" 스펙을 다시 보니 **앱이 이미 로컬에서 TTS·경보음을 10초 간격으로 반복하고
  있어서 서버가 같은 메시지를 30초마다 다시 보내도 실질적으로 더해주는 게 없다는 걸
  사용자 질문으로 확인함**(지금도 계속 울리는 경보와 재발송이 같은 개념 아니냐는 지적).
  서버 재발송이 의미 있는 유일한 경우는 **최초 발송 자체가 폰에 안 닿은 경우**(로컬 반복 알람은
  발송이 도착해야 시작되므로) — 근데 `send_fcm()`을 다시 보니 그 경우에 대한 재시도가
  0번이었음(실패하면 로그만 찍고 끝). 그래서 원래 계획했던 재발송 스케줄러(상태 추적,
  ACK 엔드포인트, 엣지 PC를 폰이 호출 가능하게 여는 것까지 필요해 "규모 큼"이었음) 대신,
  **`send_fcm()` 자체에 실패 시 재시도만 추가**하는 훨씬 작은 수정으로 결론.
  `_try_send_fcm_once()`로 "한 번 시도"를 분리해 (성공 여부, 재시도할 가치 있는지, 로그)를
  반환하게 하고, 재시도 가치는 실패 종류로 가름 — 네트워크 순단·429·5xx는 재시도(최대
  `FCM_MAX_ATTEMPTS=3`번, `FCM_RETRY_BACKOFF_SEC=1.5`초 간격), 인증 파일 없음(`FileNotFoundError`)이나
  400/401/403/404는 재시도해도 안 풀리는 문제라 즉시 포기. 메인 프레임 루프에서 동기 호출되는
  함수라(클립 저장과 같은 이유로 스레드 분리는 아직 안 함) 재시도 총 대기 시간을 짧게 유지.
  몽키패치로 5가지 시나리오(네트워크 2번 실패 후 성공/500 계속/401 즉시 포기/인증파일
  없음/토큰 빈값) 검증 완료 — 커밋 전엔 실제 테스트 파일로 남기지 않음(이 저장소 관례,
  세션 중 검증만 하고 넘어감).
- [x] **주의(헬멧·보안경) 등급은 관리자 폰 FCM 발송 자체를 끔 — 기획안 재확인으로 발견한
  진짜 과보고 방지 조항 (2026-09-21, 사용자 지적)** — 위 재시도 작업을 "원래 계획했던
  백그라운드 스케줄러·ACK·서비스워커 버튼이 불필요해졌다"고 설명했더니, 사용자가 자기
  기획안을 다시 확인하고서 그 설명이 "앱이 알람을 스스로 끄는가"에 초점이 가 있는데
  원래 기획 의도는 과보고 자체를 막으려는 것 아니었냐고 지적함 — 정확한 지적이었다.
  기획안(`SOP_행동인식_시스템_기획안_세정기_0917.pdf`)
  5.5절 등급표를 다시 읽어보니 "오경보는 작업 방해와 시스템 신뢰 저하를 유발하므로 **관리자
  푸시는 중대 편차에 한정한다**"는 조항이 있고, 주의(Major) 행에는 "안내음 + 이벤트 목록
  기록(**푸시 미발송**)"이라고 명시돼 있었다. 그런데 실제 `on_violation_confirmed`는
  `args.fcm_token`만 있으면 등급과 무관하게 4규칙 전부 `send_fcm`을 호출하고 있었다
  (`FCM_LEVEL_BY_RULE`은 앱 배지 표시용일 뿐 발송 여부를 안 가르는 버그, 2026-09-15 FCM
  연동 때부터 있었음). `level == "중대"`일 때만 `send_fcm`을 호출하도록 게이트 추가 —
  주의 위반은 이제 `events.jsonl`엔 그대로 남지만 관리자 폰에는 안 간다(해제 신호는 기존
  설계대로 그대로 둠 — 확정이 애초에 안 갔으면 앱에서 매칭될 게 없어 조용히 무시되므로
  게이트 불필요). 몽키패치로 crew/zone(중대)은 발송, helmet/glasses(주의)는 미발송 확인.
  **교훈**: "재발송을 어떻게 줄이거나 대체하나"에만 집중하다 기획안의 "애초에 이 등급은
  보내지 않는다"는 더 근본적인 조항을 놓쳤다 — 기획안을 재확인해야 할 때는 방금 손댄
  기능 주변만 보지 말고 관련 절 전체를 다시 읽을 것. "일반(STEP 시간 초과)"은 아직 대상
  규칙 자체가 없어 미해당
...
- [x] **헬멧·보안경(PPE)을 주의→중대 등급으로 재분류 (2026-09-22)** — 본사 SOP 문서 확인 결과
  헬멧·보안경 미착용도 급소포인트(치명 위험)로 판단돼, 기획안 5.5절 등급표(중대=2인1조·
  안전구역, 주의=헬멧·보안경)와 달리 4규칙 전부를 중대로 취급하기로 정책 변경. `FCM_LEVEL_BY_RULE`
  (`webcam_sop.py`)에서 `helmet`/`glasses`를 `"주의"`→`"중대"`로 수정 — 2026-09-21에 추가된
  "`level == "중대"`일 때만 `send_fcm` 호출" 게이트(위 "알림 설계" 섹션 참고)는 그대로 둔 채
  매핑값만 바꿨으므로, 이제 헬멧·보안경 위반도 실제로 관리자 폰까지 발송된다(그 전엔
  events.jsonl에만 기록되고 폰에는 안 갔음). `rtauto_sop_android` 쪽은 `level` 값을 그대로
  신뢰해 재생 분기를 태우는 구조라 앱 코드 변경은 불필요 — 다만 그 레포의 CLAUDE.md에 남아있는
  "헬멧은 주의 등급" 예시 설명은 낡은 내용이 됐으니 별도로 갱신 필요. **기획안 5.5절 등급표
  자체도 이 정책과 어긋나게 됐으니, 문서(장비_구매_기획안...) 쪽도 다음에 같이 갱신할 것.**
