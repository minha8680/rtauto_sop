...
전부 미구현. `push_server.py`(실제 알림 채널)는 이미 있으니 언제든 착수 가능하다:
1. **등급별 차등 발송** — 기획안 그림 4 목업에 이미 중대/일반/주의 3등급이 있음. 중대(2인1조)만
   30초 재발송, 주의(보호구)는 1회, 일반(표준시간 초과)은 이벤트 목록에만 기록.
   **규모가 큼** — `push_server.py`에 백그라운드 재발송 스케줄러, 확인(ACK) 엔드포인트,
   알림에 "확인" 액션 버튼(서비스워커), 이벤트 이력 저장소까지 새로 필요.
2. **반복(flapping) 억제** — 해제 후 N분 내 같은 유형 재발생 시 새 알림 대신 누적 카운트,
   임계 초과 시 "반복 발생" 요약 1회. **규모가 작음** — `webcam_sop.py`의 `send_push()` 호출부만
   쿨다운 래퍼로 감싸면 됨, 서버 쪽은 안 건드림. 1번보다 먼저 하기 좋은 후보(2026-09-11 판단)
...
- [ ] (선택) 알림 고도화 — 등급별 차등 발송, 반복 억제 (위 "알림 설계" 섹션 참고. 반복 억제가
  규모 작아 먼저 하기 좋은 후보, 등급별 차등은 서버 쪽 스케줄러·ACK까지 필요해 더 큼)
