...
채널)는 이미 있다:
1. **등급별 차등 발송 — 재검토 결과 대부분 이미 충족, 나머지는 재시도 보강으로 축소
   (2026-09-21)** — 기획안 그림 4 목업의 중대/일반/주의 3등급 중 **주의(보호구) 1회 발송은
   이미 그대로 동작**, **일반(표준시간 초과)은 대상 규칙 자체가 아직 없음**(SOP 필요).
   유일하게 미구현이던 "중대는 확인할 때까지 30초 간격 재발송"은, 앱이 이미 로컬에서
   TTS·경보음을 반복 재생하고 있어 **서버가 같은 메시지를 다시 보내는 게 실질적 이득이
   거의 없다는 걸 확인**하고 방향을 바꿈 — 서버 재발송이 의미 있는 유일한 경우(최초 발송
  자체가 폰에 안 닿음)에 대응해 `send_fcm()`에 실패 시 재시도만 추가(`_try_send_fcm_once`,
   최대 3회). 원래 계획했던 백그라운드 스케줄러·ACK 엔드포인트·서비스워커 액션 버튼은
   불필요해짐 — 상세는 아래 "다음 작업" 2026-09-21 항목.
...
- [x] **알림 등급별 차등 발송 재검토 → `send_fcm()` 재시도 보강으로 축소 (2026-09-21)** —
  ...
  사용자 질문으로 확인함**("지금도 계속 경보가 울리는데 재발송하고 같은 개념 아니냐"). 서버
  재발송이 의미 있는 유일한 경우는 **최초 발송 자체가 폰에 안 닿은 경우**(로컬 반복 알람은
  ...
  **`send_fcm()` 자체에 실패 시 재시도만 추가**하는 훨씬 작은 수정으로 결론.
  ...
  세션 중 검증만 하고 넘어감). "주의(헬멧·보안경) 1회 발송"은 이미 스펙대로 동작 중이라
  손대지 않음, "일반(STEP 시간 초과)"은 아직 대상 규칙 자체가 없어 미해당
