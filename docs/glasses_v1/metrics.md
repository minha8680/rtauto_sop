...
1. [x] 로컬 웹캠 실시간 테스트 (착용/미착용 전환, conf 임계값 튜닝) — `webcam_glasses.py`
2. [x] **`webcam_sop.py`에 통합 완료(2026-09-17)** — helmet과 완전히 독립된 규칙으로 추가
   (`--glasses-hold`/`--glasses-clear`/`--no-glasses`). 매칭 로직(person 박스 중심점 포함
   여부로 "검출=착용" 오류 방지)은 `match_ppe_to_persons()`로 일반화해 helmet과 공유
3. **실물 검증에서 한계 발견(2026-09-17)** — 실제 사용 중인 풀페이스 단일렌즈형(스플래시
   고글) 실물과 그 사진을 프린트한 것 모두 인식 실패. 학습 데이터셋(Roboflow PPE Combined
   Model)이 이 스타일을 충분히 포함하지 않을 가능성 — `CONF` 낮춰서 재확인 필요, 그래도
   안 되면 이 스타일 사진을 추가로 모아 파인튜닝 필요(다음 실험 대상, 아직 착수 전)
